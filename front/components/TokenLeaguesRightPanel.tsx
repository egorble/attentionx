...
const TokenLeaguesRightPanel: React.FC<RightPanelProps> = ({ isMobile }) => {
  ...
  return (
    <aside className={isMobile
      ...
      : "w-64 h-screen fixed right-0 top-0 bg-zinc-950 border-l border-zinc-900 p-3 hidden xl:flex flex-col space-y-3 z-40 overflow-y-auto"
    }>

      {/* Real-time Leaderboard */}
      <div className="bg-zinc-900 rounded-[32px] p-4 flex-1 flex flex-col min-h-[300px] relative overflow-hidden">
        {selectedPlayer ? (
          <div className="flex flex-col h-full animate-in slide-in-from-right-8 duration-300">
            <div className="flex items-center mb-4">
              <button onClick={() => setSelectedPlayer(null)} className="p-1 -ml-1 hover:bg-zinc-800 rounded-full transition-colors mr-2">
            ...
            <p className="text-[10px] text-zinc-400 font-bold uppercase mb-3 px-1">Selected Tokens</p>
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
              {selectedPlayer.tokens.map(tokenId => {
                ...
                if (!t) return null;
                return (
                  <div key={t.id} className="flex items-center gap-2 p-2 rounded-2xl bg-zinc-950 border border-zinc-800">
            ...
            <div className="flex-1 overflow-y-auto space-y-1.5 pr-1 custom-scrollbar">
              {leaderboard && leaderboard.length > 0 ? (
                leaderboard.slice(0, 50).map((entry, i) => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center justify-between px-2 py-2 rounded-2xl transition-colors cursor-pointer ${isYou ? 'bg-zinc-800 border border-[#9333ea]/30' : 'bg-zinc-950 hover:bg-zinc-800'
                        }`}
                    >
                      <div className="flex items-center min-w-0 flex-1">
                        <span className={`text-[10px] font-black shrink-0 mr-2 w-4 text-center ${i === 0 ? 'text-[#9333ea]' : i < 3 ? 'text-white' : 'text-zinc-500'
                          }`}>
                          {i + 1}
                        </span>
                        <p className={`text-[11px] font-semibold truncate ${isYou ? 'text-[#9333ea]' : 'text-zinc-300'
                          }`}>
                ...
                <div className="flex items-center justify-center h-full">
                  <p className="text-[10px] text-zinc-500 text-center">Waiting for selection phase to complete...</p>
                </div>
              )}
            </div>
          </div>
      ...
      <div className={`bg-zinc-900 rounded-[32px] p-4 shrink-0 ${isMobile ? 'hidden' : ''}`}>
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-white font-bold text-xs uppercase tracking-wider">Referral Program</h3>
          <UserPlus className="w-4 h-4 text-[#9333ea]" />
        </div>
        <p className="text-zinc-400 text-[10px] mb-3 leading-tight">
          Earn <span className="text-[#9333ea] font-bold">10%</span> from every pack your friends buy.
        </p>

        {isConnected && (
          <div className="flex gap-2 mb-3">
            <div className="flex-1 bg-zinc-950 rounded-xl p-2 text-center">
              <p className="text-[9px] text-zinc-500 uppercase font-bold mb-0.5">Refs</p>
              <p className="text-white font-bold font-mono text-sm">{referralStats.count}</p>
            </div>
            <div className="flex-1 bg-zinc-950 rounded-xl p-2 text-center">
              <p className="text-[9px] text-zinc-500 uppercase font-bold mb-0.5">Earned</p>
        ...
        <div className="relative">
          <input
            ...
            readOnly
            className="w-full bg-zinc-950 text-zinc-400 text-[10px] px-3 py-2.5 pr-14 rounded-full font-mono focus:outline-none truncate"
          />
          <button
            onClick={handleCopy}
